...
function buildFloristStory(florist: any, styles: any[], deliveryAreas: any[]) {
  ...
  const styleText =
    styles.length > 0
      ? styles.slice(0, 3).join(", ").toLowerCase()
      : "personliga";
  const deliveryModel = florist.delivery_model
    ? String(florist.delivery_model).toLowerCase()
    : "lokalt";
  const firstPrice = deliveryAreas.find((area: any) => area?.price)?.price;
  const priceText = firstPrice
    ? ` och leveranspriset börjar från ${formatPrice(firstPrice)}`
    : "";
  const radiusText = florist.delivery_radius_km
    ? ` inom upp till ${florist.delivery_radius_km} km`
    : "";
...
export default async function FloristProfilePage({ params }: PageProps) {
      ...
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center px-6">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center border border-neutral-200">
          ...
          <div className="space-y-3">
            <Link
              href="/"
              className="block w-full rounded-2xl bg-black text-white py-3 font-medium hover:opacity-90 transition"
            >
              Tillbaka till feed
            </Link>
            <Link
              href="/florists"
              className="block w-full rounded-2xl border border-neutral-300 py-3 font-medium hover:bg-neutral-100 transition"
            >
              Visa alla florister
            </Link>
          </div>
  ...
  const profileImage =
    florist.profile_image_url && florist.profile_image_url !== logoImage
      ? florist.profile_image_url
      : "https://placehold.co/400x400/png";

  const servicePortfolio = flattenServicePortfolio(
    florist.service_portfolio_items,
  );
  const generalPortfolio = normalizeGeneralPortfolio(
    florist.general_portfolio_items,
  );
  const oldPortfolio = fallbackPortfolio(florist.portfolio_images);
  const portfolioItems =
    servicePortfolio.length || generalPortfolio.length
      ? [...servicePortfolio, ...generalPortfolio]
      : oldPortfolio;
  ...
  const coverImage =
    ...
    "https://placehold.co/1400x500/png";
  ...
  const deliveryAreas = asArray(
    florist.delivery_areas || florist.coverageAreas,
  );
      ...
      <main className="max-w-6xl mx-auto px-6 pb-20 -mt-10 relative z-10">
        <section className="bg-white rounded-[32px] shadow-xl border border-neutral-200 p-6 md:p-10 mb-8">
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-1">
              ...
              {florist.bio && (
                <p className="text-neutral-500 leading-7 mt-4 max-w-3xl">
                  {florist.bio}
                </p>
              )}

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-8 text-sm">
                {florist.legal_business_name && (
                  <InfoCard
                    label="Företagsnamn"
                    value={florist.legal_business_name}
                  />
                )}
                {florist.city && <InfoCard label="Stad" value={florist.city} />}
                {florist.country && (
                  <InfoCard label="Land" value={florist.country} />
                )}
                {florist.price_level && (
                  <InfoCard label="Prisnivå" value={florist.price_level} />
                )}
                {florist.minimum_booking_value && (
                  <InfoCard
                    label="Minsta bokningsvärde"
                    value={formatPrice(florist.minimum_booking_value)}
                  />
                )}
                {florist.years_in_business && (
                  <InfoCard
                    label="År i branschen"
                    value={`${florist.years_in_business} år`}
                  />
                )}
                {florist.team_size && (
                  <InfoCard
                    label="Team"
                    value={`${florist.team_size} florist(er)`}
                  />
                )}
              </div>
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-3">
                <Link
                  ...
                  className="rounded-2xl bg-black text-white px-6 py-3 text-center font-semibold hover:opacity-90 transition"
                >
                  Kontakta floristen via FloristSocial
                </Link>
                <Link
                  href={`/orders/new?floristId=${florist.id}`}
                  className="rounded-2xl border border-neutral-300 px-6 py-3 text-center font-semibold hover:bg-neutral-50 transition"
                >
                  Starta beställning
                </Link>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <Panel
              title="Specialiteter och tjänster"
              subtitle="Välj en tjänst för att starta beställning eller se exempelbilder."
            >
              {services.length === 0 ? (
                  ...
                  {services.map((service: any, index: number) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                          <div>
                            ...
                            <p className="text-sm text-neutral-500">
                              Exempel och beställningsväg för{" "}
                              {serviceName.toLowerCase()}.
                            </p>
                          </div>
                          <Link
                            href={serviceLink(florist.id, serviceName)}
                            className="rounded-2xl bg-pink-600 text-white px-5 py-3 text-sm font-semibold hover:bg-pink-700 transition"
                          >
                            Beställ {serviceName}
                          </Link>
                        </div>
                        ...
                        {items.length === 0 ? (
                          <p className="text-sm text-neutral-500">
                            Inga bilder uppladdade för denna tjänst ännu.
                          </p>
                        ) : (
            ...
            <Panel
              title="Portfolio"
              subtitle="Bilder, priser och beskrivningar från floristen."
            >
            ...
            <Panel
              title="Stil"
              subtitle="Klicka på en stil för att starta en beställning i den stilen."
            >
              {styles.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-3">
                  {styles.map((style: any, index: number) => {
                    ...
                    return (
                      <Link
                        key={`${styleName}-${index}`}
                        href={styleLink(florist.id, styleName)}
                        className="rounded-full bg-neutral-100 text-neutral-700 border border-neutral-200 px-4 py-2 text-sm font-medium hover:bg-neutral-900 hover:text-white transition"
                      >
                        {styleName}
                      </Link>
                    );
          ...
          <aside className="space-y-8">
            <Panel
              ...
              subtitle="Pris baserat på avstånd från butiken."
            >
                  ...
                  {deliveryAreas.map((area: any, index: number) => (
                    ...
                    >
                      <div className="font-semibold text-neutral-900">
                        {area?.city || florist.city || "Leveransområde"}
                      </div>
                      <div className="text-sm text-neutral-600 mt-2">
                        Avstånd från butik upp till{" "}
                        {area?.radius || florist.delivery_radius_km || "?"} km
                        är {formatPrice(area?.price) || "ej angivet"}.
                      </div>
                      {area?.area && (
              ...
              {openingHours.length === 0 ? (
                ...
                <div className="space-y-2">
                  {openingHours.map((day: any, index: number) => (
                    <div
                      key={day?.id || index}
                      className="flex justify-between gap-4 text-sm border-b border-neutral-100 pb-2"
                    >
                      <span className="font-medium">
                        {day?.dayLabel || day?.day || `Dag ${index + 1}`}
                      </span>
            ...
            <Panel
              title="Stängda dagar"
              subtitle="Kommande två månader. Klickbar redigering kopplas senare till floristens adminläge."
            >
              <ClosedDaysCalendar months={months} closedDates={closedDates} />
            </Panel>
          </aside>
        </div>
      </main>
...
function PortfolioCard({ item, floristId }: { item: any; floristId: string }) {
  return (
    <Link
      href={orderLink(floristId, item)}
      className="overflow-hidden rounded-[28px] bg-white border border-neutral-200 shadow-sm group hover:shadow-lg transition"
    >
      ...
      <div className="p-5">
        <div className="text-xs font-semibold uppercase tracking-wide text-pink-600 mb-2">
          {item.serviceName || "Portfolio"}
        </div>
        <h3 className="font-bold text-lg text-neutral-900">
          {item.title || "Beställ liknande"}
        </h3>
        {item.price && (
        ...
        {item.hashtags && (
          <p className="mt-3 text-sm text-pink-600 break-words">
            {item.hashtags}
          </p>
        )}
        ...
        <div key={month.label}>
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {month.days.map((day: any) => {
              const isClosed = closedSet.has(day.value);
              return (
                <button
                  key={day.value}
                  type="button"
                  className={`aspect-square rounded-xl text-sm font-medium border ${isClosed ? "bg-red-500 text-white border-red-500" : "bg-white text-neutral-700 border-neutral-200 hover:border-neutral-400"}`}
                >
                  {day.day}
                </button>
              );
