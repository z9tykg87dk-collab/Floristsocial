...
const demoProducts = [
  {
    ...
    price: "från 599 kr",
    image:
      "[https://images.unsplash.com/photo-1526047932273-341f2a7631f9?q=80&w=900&auto=format&fit=crop](https://images.unsplash.com/photo-1526047932273-341f2a7631f9?q=80&w=900&auto=format&fit=crop)",
  },
  {
    title: "Röda rosor",
    price: "från 499 kr",
...
export default async function FloristSocialProfilePage({ params }: PageProps) {
  ...
  if (!florist) {
    return (
      <>
        ...
        <main className="grid min-h-screen place-items-center bg-[#f7f4ef] px-5 text-stone-900">
          {" "}
          <section className="max-w-md rounded-[32px] bg-white p-8 text-center shadow-xl ring-1 ring-stone-200">
            {" "}
            <Store className="mx-auto text-stone-300" size={48} />{" "}
            <h1 className="mt-4 text-3xl font-bold">Florist hittades inte</h1>{" "}
            ...
            <div className="mt-6 grid gap-3">
              ...
              <Link
                href="/marketplace"
                className="rounded-2xl border border-stone-300 bg-white px-5 py-3 text-sm font-bold text-stone-900"
              >
                Till marketplace
              </Link>{" "}
            </div>{" "}
  ...
  const deliveryList = deliveryAreas.length
    ? deliveryAreas
    : fallbackDeliveryAreas;
  ...
  const previewPortfolio = portfolio.length
    ? portfolio.slice(0, 3)
        ...
        <section className="mx-auto max-w-[1680px] px-3 pt-3 sm:px-5 lg:px-8 lg:pt-6">
          <div className="overflow-hidden rounded-[28px] bg-white shadow-sm ring-1 ring-stone-200 lg:rounded-[36px]">
            ...
            <div className="relative px-4 pb-6 sm:px-6 lg:px-12 lg:pb-9">
              <button
                className="absolute right-5 top-5 grid h-12 w-12 place-items-center rounded-full bg-white text-stone-900 shadow-md ring-1 ring-stone-200 lg:hidden"
                type="button"
              >
                <Heart size={22} />
              </button>
              ...
              <div className="grid gap-5 lg:grid-cols-[190px_1fr_auto] lg:items-end xl:grid-cols-[220px_1fr_auto]">
                <div className="relative -mt-14 h-32 w-32 shrink-0 overflow-hidden rounded-full border-[6px] border-white bg-white shadow-2xl sm:-mt-16 sm:h-40 sm:w-40 lg:-mt-24 lg:h-44 lg:w-44">
                  ...
                  <span
                    className={`absolute bottom-4 right-4 h-5 w-5 rounded-full border-2 border-white ${today.openNow ? "bg-emerald-500" : "bg-red-500"}`}
                  />
                </div>

                <div className="min-w-0 lg:pb-2">
                  <h1 className="text-4xl font-[900] leading-none tracking-tight sm:text-5xl xl:text-6xl">
                    {name}
                  </h1>
                  <div className="mt-4 flex flex-col gap-2 text-sm text-stone-700 md:flex-row md:flex-wrap md:items-center md:gap-x-5">
                    <span className="inline-flex items-center gap-2 font-bold">
                      <span
                        className={`h-3 w-3 rounded-full ${today.openNow ? "bg-emerald-500" : "bg-red-500"}`}
                      />
                      {today.openNow
                  ...
                  <p className="mt-2 text-sm text-stone-500">
                    Vanligtvis svarar inom några minuter
                  </p>
                  <Link
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${name} ${getAddress(florist)}`)}`}
                    target="_blank"
                    className="mt-4 inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-stone-200 bg-white px-4 text-sm font-bold text-stone-900 shadow-sm hover:bg-stone-50"
                  >
        ...
        <section className="mx-auto max-w-[1680px] space-y-5 px-3 py-5 sm:px-5 lg:px-8">
          <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200 lg:p-6">
            <div className="grid gap-5 lg:grid-cols-[320px_1fr] lg:items-center">
              ...
              <div>
                <h2 className="text-2xl font-[900] tracking-tight lg:text-3xl">
                  Om {name}
                </h2>
              ...
              <div className="grid grid-cols-3 gap-3">
                {previewPortfolio.map((item) => (
                  <div
                    ...
                    className="aspect-[4/5] overflow-hidden rounded-2xl bg-stone-100"
                  >
                    <img
                      src={item.image}
                      alt={item.title}
          ...
          <section
            id="portfolio"
            className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200 lg:p-6"
          >
            <SectionTop
              title="Serviceportfolio"
              subtitle="Bilder, priser och exempel från floristens registrerade tjänster."
            />
            {portfolio.length === 0 ? (
          ...
          <div className="grid gap-5 xl:grid-cols-[1fr_1fr_2fr]">
            <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200 lg:p-6">
              <h2 className="flex items-center gap-2 text-xl font-[900] tracking-tight">
            ...
            <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200 lg:p-6">
              ...
              <Link
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${name} ${getAddress(florist)}`)}`}
                target="_blank"
                className="mt-4 inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-stone-200 bg-white px-4 text-sm font-bold text-stone-900 hover:bg-stone-50"
              >
                <MapPin size={16} /> Visa på kartan
              </Link>
            </section>
            ...
            <section className="rounded-[28px] bg-pink-50 p-6 ring-1 ring-pink-100 lg:flex lg:items-center lg:justify-between lg:gap-8">
              <div className="flex items-start gap-4">
                <div className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-white text-pink-600 shadow-sm">
                  <ShieldCheck size={34} />
                </div>
                <div>
                  <h2 className="text-2xl font-[900] tracking-tight text-pink-700">
...
function Action({
  ...
  return (
    <Link
      href={href}
      className={`inline-flex h-12 items-center justify-center gap-2 rounded-2xl px-5 text-sm font-extrabold transition ${primary ? "bg-gradient-to-r from-pink-600 to-fuchsia-500 !text-white shadow-lg shadow-pink-600/20 hover:from-pink-700 hover:to-fuchsia-600" : "border border-pink-300 bg-white !text-pink-700 hover:bg-pink-50"}`}
    >
      {icon}
      {children}
    </Link>
...
function InfoCard({
  ...
  return (
    <div className="flex items-center gap-4 rounded-[22px] bg-white p-5 ring-1 ring-stone-200">
      <div className="text-stone-900">{icon}</div>
      <div>
        <div className="font-black">{label}</div>
        <div className="mt-1 text-sm text-stone-600">{value}</div>
      </div>
    </div>
  );
}
...
function PinkTag({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full bg-pink-50 px-4 py-2 text-xs font-bold text-pink-700 ring-1 ring-pink-100">
      {children}
    </span>
  );
}
  ...
  return (
    <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200">
      <div className="mb-4 flex items-start justify-between gap-4">
        <h2 className="text-xl font-[900] tracking-tight">{title}</h2>
        {href && link && (
          <Link
            href={href}
            className="shrink-0 text-sm font-black text-pink-600"
          >
            {link} →
          </Link>
        )}
...
function PortfolioTile({ item }: { item: PortfolioItem }) {
  return (
    <article className="overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-stone-200">
      ...
      <div className="p-4">
        <div className="text-xs font-black uppercase tracking-wide text-pink-600">
          {item.serviceName}
        </div>
    ...
    <div className="mt-4 space-y-2">
      {normalized.map((day) => (
        ...
        >
          ...
          <span
            className={day.closed ? "font-bold text-red-600" : "text-stone-600"}
          >
...
function TagGroup({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
      ...
      {items.length === 0 ? (
        <p className="text-sm text-stone-500">Inget registrerat ännu.</p>
      ) : (
...
function TrustItem({ text }: { text: string }) {
  return (
    <div className="flex items-start gap-2">
      <Check size={18} className="mt-0.5 shrink-0 text-pink-600" /> {text}
    </div>
  );
}
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="rounded-3xl border border-dashed border-stone-300 bg-stone-50 p-10 text-center text-stone-500">
      <CalendarDays className="mx-auto mb-3 text-stone-300" size={34} />
      {text}
    </div>
  );
}
