...
const paymentMethods = ["Betalkort med Stripe", "Klarna", "Swish", "Apple Pay", "Google Pay"];
...
const priceOptions = ["400 kr", "500 kr", "650 kr", "750 kr", "1.000 kr", "1.250 kr", "1.500 kr", "2.000 kr", "Annat pris"];
const extraProducts = ["Choklad 50 kr", "Vas 150 kr", "Nalle 120 kr", "Ballong 75 kr", "Extra kort 35 kr", "Lyxigare inslagning 90 kr"];
const styleOptions = ["Romantiskt", "Modernt", "Klassiskt", "Lyxigt", "Färgstarkt", "Säsongsbaserat", "Pollenfri", "Floristens val"];
...
export default function PrivateCustomerAndGuestOrderPage() {
  ...
  const productPrice = selectedPrice === "Annat pris" ? priceToNumber(customPrice) : priceToNumber(selectedPrice);
  const extrasTotal = selectedExtras.reduce((total, item) => total + priceToNumber(item), 0);
  const cardTotal = cardPrice === "Inget kort" ? 0 : priceToNumber(cardPrice);
  const deliveryTotal = deliveryMethod === "Leverans" ? Number(deliveryFee || 0) : 0;
  const subtotal = productPrice * quantity;
  const orderTotal = subtotal + extrasTotal + cardTotal + deliveryTotal;
  const isBelowMinimum = subtotal < MINIMUM_ORDER_VALUE;

  const orderCreatedText = useMemo(() => `${createdAt}, ${city}`, [createdAt, city]);

  function toggleExtra(product: string) {
    setSelectedExtras((items) => items.includes(product) ? items.filter((item) => item !== product) : [...items, product]);
  }
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (isBelowMinimum) {
      setSubmitMessage(`⚠️ Minsta ordervärde är ${MINIMUM_ORDER_VALUE} kr exklusive leverans och tillval. Välj minst 400 kr i produktvärde.`);
      return;
    }

    setSubmitMessage(`✅ Beställning ${orderNumber} skapad. Kunden betalar ${orderTotal} kr. PDF-kvitto med moms ska skickas direkt efter genomförd betalning.`);
  }
      ...
      <section className="relative overflow-hidden px-5 py-10 md:px-10 lg:px-16">
        ...
        <div className="relative mx-auto max-w-7xl space-y-8">
          <header className="max-w-4xl">
            ...
            <h1 className="text-4xl font-semibold tracking-tight md:text-6xl">Beställ blommor som privatkund eller gäst.</h1>
            <p className="mt-5 text-lg leading-8 text-stone-600">
              Kunden ska kunna beställa snabbt utan konto, men kan även skapa ett privatkonto för orderhistorik, sparade adresser, favoritflorister och snabbare checkout senare.
            </p>
            <p className="mt-3 text-base leading-7 text-stone-600">
              Minsta produktvärde är <strong>400 kr</strong>. Kunden betalar produkt, extra produkter, hälsningskort och eventuell leverans. Kortavgift eller Stripeavgift visas inte för kunden.
            </p>
          </header>
          ...
          <form onSubmit={handleSubmit} className="grid gap-8 lg:grid-cols-[1fr_380px]">
            <div className="space-y-8">
              <Card>
                <SectionHeader icon={<User size={20} />} title="1. Beställarens uppgifter" description="Dessa uppgifter behövs för orderbekräftelse, betalning och kontakt vid frågor." />
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <label className="block">
                    ...
                    <select value={city} onChange={(event) => setCity(event.target.value)} className="h-12 w-full rounded-2xl border border-stone-200 bg-white px-4 outline-none transition focus:border-stone-500">
                      {swedishCities.map((item) => <option key={item} value={item}>{item}</option>)}
                    </select>
                  </label>
                </div>

                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  <Field required name="buyerName" label="Beställarens namn" placeholder="Namn Efternamn" icon={<User size={18} />} />
                  <Field required name="buyerPhone" label="Beställarens telefonnummer" placeholder="070 000 00 00" type="tel" icon={<Phone size={18} />} />
                  <Field required name="buyerEmail" label="Beställarens e-post" placeholder="namn@email.se" type="email" icon={<Mail size={18} />} />
                  <label className="flex items-start gap-3 rounded-2xl bg-stone-50 p-4 text-sm text-stone-700">
                    <input type="checkbox" checked={createAccount} onChange={(event) => setCreateAccount(event.target.checked)} className="mt-1 h-4 w-4 accent-stone-900" />
                    <span>Skapa privatkonto efter köpet för orderhistorik och snabbare checkout</span>
                  </label>
                </div>

                {createAccount && <PasswordFields generatedPassword={generatedPassword} onGenerated={setGeneratedPassword} />}
              </Card>

              <Card>
                <SectionHeader icon={<MapPin size={20} />} title="2. Mottagarens uppgifter" description="Floristen eller budet måste kunna hitta mottagaren och kontakta vid portproblem eller leveransfrågor." />

                <div className="grid gap-4 md:grid-cols-2">
                  <Field required name="recipientName" label="Mottagarens namn" placeholder="Namn Efternamn" icon={<User size={18} />} />
                  <Field required name="recipientPhone" label="Mottagarens mobiltelefonnummer" placeholder="070 000 00 00" type="tel" icon={<Phone size={18} />} />
                  <Field name="recipientEmail" label="Mottagarens e-post" placeholder="valfritt@email.se" type="email" icon={<Mail size={18} />} />
                  <Field required name="recipientAddress" label="Mottagarens fullständiga adress" placeholder="Gatuadress och nummer" icon={<Home size={18} />} />
                  <Field required name="recipientPostalCode" label="Postnummer" placeholder="113 50" />
                  <Field required name="recipientCity" label="Ort" placeholder="Stockholm" />
                  <Field name="doorCode" label="Portkod" placeholder="Ex. 1234" />
                  <Field name="floor" label="Våning" placeholder="Ex. 3 tr" />
                  <Field name="apartmentNumber" label="Lägenhetsnummer" placeholder="Ex. 1202" />
                  <Field name="careOf" label="C/O" placeholder="Ex. c/o Andersson" />
                </div>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  ...
                  <label className="flex items-start gap-3 rounded-2xl bg-stone-50 p-4 text-sm text-stone-700">
                    <input type="checkbox" checked={anonymousDelivery} onChange={(event) => setAnonymousDelivery(event.target.checked)} className="mt-1 h-4 w-4 accent-stone-900" />
                    <span>Anonym leverans</span>
                  </label>
                </div>

                <Textarea name="deliveryInstructions" label="Leveransinstruktioner" placeholder="Portinformation, mottagning, ring före leverans, lämnas hos granne, osv." />
              </Card>

              <Card>
                <SectionHeader icon={<ShoppingBag size={20} />} title="3. Beställning" description="Välj typ av arrangemang, pris, antal, stil och eventuella tillval." />
                ...
                <div className="mt-8">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    {styleOptions.map((style) => (
                      <PillButton key={style} active={selectedStyle === style} onClick={() => setSelectedStyle(style)} variant="pink">{style}</PillButton>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-8 grid gap-4 md:grid-cols-3">
                  ...
                  {selectedPrice === "Annat pris" && <ControlledInput label="Annat pris" value={customPrice} onChange={setCustomPrice} placeholder="Minst 400 kr" />}
                  <label className="block">
                  ...
                  <SelectField required name="cardPrice" label="Hälsningskort" options={cardOptions} value={cardPrice} onChange={setCardPrice} />
                </div>

                {isBelowMinimum && (
                  <div className="mt-4 rounded-2xl bg-red-50 p-4 text-sm font-medium text-red-700">
                    Minsta produktvärde är {MINIMUM_ORDER_VALUE} kr. Leverans och extra tillval räknas ovanpå detta.
                  </div>
                )}

                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  <Textarea name="cardText" label="Hälsningskort text" placeholder="Till ex.: Hälsningar från Morbror Johan" />
                  <Textarea name="specialRequests" label="Specialönskemål" placeholder="Till ex.: färgglad, pollenfri, vita rosor, säsongens blommor ..." />
                </div>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-2xl bg-stone-50 p-5">
                    ...
                    <p className="mt-1 text-sm text-stone-600">Kunden kan klicka och lägga till extra produkter.</p>
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      {extraProducts.map((product) => (
                        <PillButton key={product} active={selectedExtras.includes(product)} onClick={() => toggleExtra(product)} variant="pink">{product}</PillButton>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <Card>
                <SectionHeader icon={<Truck size={20} />} title="4. Leverans eller avhämtning" description="Kunden kan välja leverans eller avhämtning från butiken." />

                <div className="grid gap-4 md:grid-cols-3">
                  <SelectField required name="deliveryMethod" label="Leveransmetod" options={deliveryMethods} value={deliveryMethod} onChange={setDeliveryMethod} />
                ...
                {deliveryMethod === "Leverans" && (
                  <div className="mt-6 grid gap-4 md:grid-cols-2">
                    <ControlledInput label="Leveransavgift" value={deliveryFee} onChange={setDeliveryFee} placeholder="Ex. 100" />
                    <div className="rounded-2xl bg-stone-50 p-4 text-sm leading-6 text-stone-600">
                      Fraktkostnad beräknas senare automatiskt utifrån avståndet mellan levererande florist/blomsterbutik och leveransadressen.
                    </div>
                  </div>
                )}
              </Card>
              ...
              <Card>
                <SectionHeader icon={<Sparkles size={20} />} title="5. Betalning och kvitto" description="Privatkunder och gäster betalar direkt. Faktura används inte för privatpersoner i första versionen." />
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="rounded-2xl bg-emerald-50 p-4 text-sm leading-6 text-emerald-800">
                    <Check size={16} className="mr-1 inline" /> PDF-kvitto med moms ska skickas direkt efter genomförd betalning.
                  </div>
                </div>

                <div className="mt-6 rounded-2xl bg-stone-50 p-4 text-sm leading-6 text-stone-600">
                  Kunden betalar endast produkt, tillval, hälsningskort och eventuell leverans. Stripe-/kortavgift visas inte separat för kunden. Varje florist betalar sin egen del av betalningsavgiften proportionellt enligt befintlig Stripe-modell.
                </div>
              </Card>
            </div>
            ...
            <aside className="lg:sticky lg:top-8 lg:h-fit">
              <div className="rounded-3xl bg-white p-6 shadow-xl">
                ...
                <div className="mt-5 space-y-4 text-sm text-stone-700">
                  ...
                  <SummaryRow label="Kundnummer" value={createAccount ? customerNumber : "Gästbeställning"} />
                  ...
                  <SummaryRow label="Extra produkter" value={`${extrasTotal} kr`} />
                  ...
                  <SummaryRow label="Totalt att betala" value={`${orderTotal} kr`} strong />
                </div>

                <div className="mt-6 rounded-2xl bg-stone-50 p-4 text-sm leading-6 text-stone-600">
                  Exempel: Bukett 500 kr + choklad 50 kr + leverans 100 kr = kunden betalar 650 kr.
                </div>

                <div className="mt-6 flex justify-end">
                  <Button type="submit"><PackageCheck size={16} /> Skicka beställning</Button>
                </div>
              </div>
            </aside>
          </form>
    ...
    <div className="mt-6 rounded-3xl bg-stone-50 p-4">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-stone-600">Kunden kan skriva eget lösenord eller generera ett säkert lösenord.</p>
        </div>
...
function SummaryRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className="flex items-start justify-between gap-3 border-b border-stone-100 pb-3"><span className="text-stone-500">{label}</span><strong className={`text-right ${strong ? "text-lg text-stone-950" : "text-stone-900"}`}>{value}</strong></div>;
}
