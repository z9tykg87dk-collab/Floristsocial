      ...
      <main className="max-w-6xl mx-auto px-6 pb-20 -mt-10 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="space-y-8">
              ...
              {deliveryAreas.length === 0 ? (
                ...
                <div className="space-y-3">
                  {deliveryAreas.map((area: any, index: number) => (
                    <div key={area?.id || index} className="rounded-2xl border border-neutral-200 p-4 bg-neutral-50">
              ...
              {openingHours.length === 0 ? (
                ...
                <div className="space-y-2">
                  {openingHours.map((day: any, index: number) => (
                    <div key={day?.id || index} className="flex justify-between gap-4 text-sm border-b border-neutral-100 pb-2">
            ...
            <Panel title="Stängda dagar" subtitle="Kommande två månader. Klickbar redigering kopplas senare till floristens adminläge.">
...
function ClosedDaysCalendar({ months, closedDates }: { months: any[]; closedDates: any[] }) {
      ...
      {months.map((month) => (
        <div key={month.label}>
          ...
          <div className="grid grid-cols-7 gap-1">
            {month.blanks.map((_: any, index: number) => <div key={`blank-${index}`} />)}
            {month.days.map((day: any) => {
              const isClosed = closedSet.has(day.value);
              return (
                <button key={day.value} type="button" className={`aspect-square rounded-xl text-sm font-medium border ${isClosed ? "bg-red-500 text-white border-red-500" : "bg-white text-neutral-700 border-neutral-200 hover:border-neutral-400"}`}>
                  {day.day}
                </button>
              );
            })}
