...
function getPopupActionLabels(role: CalendarEngineRole): CalendarActionType[] {
  if (role === "company") {
    return [
...
function CalendarEngineCore({
  floristId,
  role = "florist",
}: {
  floristId: string;
  role?: CalendarEngineRole;
}) {
  ...
  const popupActionLabels = getPopupActionLabels(role);
  const [savedNotes, setSavedNotes] = useState<SavedCalendarNote[]>([]);

  const days = useMemo(
    () => Array.from({ length: 56 }, (_, index) => index - 14),
    [],
  );
  const selectedEvents = events.filter(
    (event) => event.offset === selectedOffset,
  );
  const selectedNotes = savedNotes.filter(
    (note) => note.offset === selectedOffset,
  );
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: 12, fontWeight: 900, color: "#e60073" }}>
            2 veckor bakåt · 6 framåt
          </span>
        </div>
            ...
            return (
              <div key={`week-${weekIndex}`} style={{ display: "contents" }}>
                ...
                {weekDays.map((offset) => {
                  ...
                  const dayNotes = savedNotes.filter(
                    (note) => note.offset === offset,
                  );
          ...
          >
            ...
            >
              <div>
                ...
                <h3
                  style={{ margin: "6px 0 0", fontSize: 28, fontWeight: 950 }}
                >
                  {selectedOffset === 0
                    ? "Idag"
                    : selectedOffset > 0
                      ? `Om ${selectedOffset} dagar`
                      : `${Math.abs(selectedOffset)} dagar sedan`}
                </h3>
                <p
                  style={{ margin: "6px 0 0", fontSize: 14, color: "#57534e" }}
                >
              ...
              >
                ...
                <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                  ...
                  {selectedNotes.map((note, index) => (
                    <div
                      key={`${note.offset}-${note.type}-${index}`}
                      style={{
                        borderRadius: 16,
                        background: "white",
                        border: "1px solid #e7e2dc",
                    ...
                    >
                      ...
                      >
                        Nästa steg: öppna order, skapa leverans eller planera
                        butiksåtgärd.
                      </p>
                      <div
                        style={{
                      ...
                      >
                        <Link href="/florist-order" style={pinkButtonStyle}>
                          Öppna order
                        </Link>
                        <Link
            ...
            >
              <Link href="/florist-order" style={pinkBlockStyle}>
                Order<span>Öppna floristorder</span>
              </Link>
              <Link href="/florist-order/delivery-note" style={greenBlockStyle}>
                Leverans<span>Skapa leveranssedel</span>
              </Link>
              {popupActionLabels.map((action) => (
                <button
                  key={action}
                  type="button"
...
const calendarActionLabels: Record<CalendarEngineRole, string[]> = {
  florist: [
    "Knapp 1", "Knapp 2", "Knapp 3", "Knapp 4", "Knapp 5", "Knapp 6",
    "Ny order", "Stängd dag", "Ny påminnelse", "Öppna orderlista",
  ],
  private: [
    "Knapp 1", "Knapp 2", "Knapp 3", "Knapp 4", "Knapp 5", "Knapp 6",
    "Ny påminnelse", "Födelsedag", "Årsdag", "Prenumeration",
  ],
  company: [
    "Knapp 1", "Knapp 2", "Knapp 3", "Knapp 4", "Knapp 5", "Knapp 6",
    "Ny beställning", "Återkommande order", "Faktura", "Påminnelse",
...
export default function CalendarEngine(props: {
  floristId?: string;
  role?: CalendarEngineRole;
  [key: string]: any;
}) {
  const { role = "florist", floristId = "internal-profile", ...rest } = props;
  return <CalendarEngineCore role={role} floristId={floristId} {...rest} />;
}
