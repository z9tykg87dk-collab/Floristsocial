  ...
  const searchMapLocation = useCallback(
    async () => {
      ...
      if (missingCountry || missingCity) {
        ...
        setMapActionMessage(
          missingCountry && missingCity
            ? "Fyll i både land och stad för att FloristSocial ska kunna hitta florister på rätt plats."
            : missingCountry
              ? "Välj land för att FloristSocial ska kunna hitta florister i denna stad."
              : "Fyll i stad för att FloristSocial ska kunna hitta florister på rätt plats.",
        );

        window.requestAnimationFrame(() => {
          const firstMissingField =
            missingCountry
              ? document.getElementById("florist-country")
              : document.getElementById("florist-city");
      ...
      try {
        ...
        for (const parameterName of [
          ...
          if (!selectedCountryCode) {
            throw new Error(
              `Landet "${normalizedCountry}" saknar en giltig landkod.`,
            );
          }
            ...
            <aside className="rounded-[30px] bg-[#fbf7f2] p-4 ring-1 ring-stone-200 md:p-5">
              <div className="grid gap-3">
                <SearchField
                  ...
                  invalid={requiredFieldErrors.country}
                />
                <SearchField
                  label="Stad"
                  value={city}
              ...
              <p className="mt-5 rounded-2xl bg-white p-4 text-xs font-semibold leading-6 text-stone-500 ring-1 ring-stone-200">
                Land och stad krävs för SÖK FLORIST. Gatuadress, postnummer och datum är valfria. FLORIST NÄRA MIG använder din aktuella position.
              </p>
            </aside>
