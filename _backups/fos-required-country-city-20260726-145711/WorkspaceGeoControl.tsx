...
export default function WorkspaceGeoControl({
  ...
  async function focusSearchOnMap(
    ...
    /*
     * Om besökaren endast har valt ett land
     * används landet som kartans sökmål.
     */
    const query =
      queryParts.join(", ") ||
      countryValue;

    const isCountryOnlySearch =
      !searchValue &&
      !cityValue &&
    ...
    if (!query) {
      setMapSearchStatus(
        "Skriv ett land, en stad, ett floristnamn, ett område, en adress eller ett postnummer.",
      );
    ...
    try {
      ...
      if (!coordinates) {
        ...
        /*
         * Om exempelvis Oslo inte hittas i det
         * valda standardlandet Sverige görs en
         * ny sökning utan landsbegränsning.
         */
        if (
          !geocodeResult.response.ok &&
          requestedCountryName &&
          !isCountryOnlySearch
        ) {
          const globalParams =
      ...
      setMapSearchStatus(
        isCountryOnlySearch
          ? `Visar ${resolvedCountryLabel || resolvedLabel}.`
          : resolvedCountryLabel
          ...
          <div>
            ...
            <div className="mt-4 grid gap-3">
              ...
              <label className="rounded-2xl bg-white px-4 py-3 ring-1 ring-stone-200">
                ...
                <p className="mt-2 text-[11px] font-semibold leading-5 text-stone-400">
                  Land är valfritt. Om staden inte hittas i valt land söker FloristSocial globalt och uppdaterar landet automatiskt.
                </p>
              </label>
