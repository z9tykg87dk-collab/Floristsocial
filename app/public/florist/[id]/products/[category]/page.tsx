...
export default async function ProductCategoryPage({ params }: PageProps) {
  ...
  const realProducts = (posts as AnyRecord[])
    .filter((post) => {
      const cat = categoryFrom(post).toLowerCase();
      const title = text(post.product_title, post.title, post.caption).toLowerCase();
      const wanted = decodedCategory.toLowerCase();
    ...
    .map((post, index) => {
      const title = text(post.product_title, post.title, post.caption) || decodedCategory;
      const price = priceFrom(post);

      return {
        id: post.id || `product-${index}`,
        title,
        price,
        image: imgFrom(post) || FALLBACK_IMAGES[index % FALLBACK_IMAGES.length],
        href: `/order/private/guest-v4?mode=directProduct&floristId=${id}&postId=${post.id || ""}&category=${encodeURIComponent(decodedCategory)}&product=${encodeURIComponent(title)}&price=${encodeURIComponent(price)}`,
      };
    });

  const fallbackProducts = Array.from({ length: 9 }).map((_, index) => {
    const price = index % 3 === 0 ? "från 500 kr" : index % 3 === 1 ? "från 750 kr" : "från 1 000 kr";
    const title = `${decodedCategory} ${index + 1}`;
    ...
    return {
      ...
      href: `/order/private/guest-v4?mode=directProduct&floristId=${id}&category=${encodeURIComponent(decodedCategory)}&product=${encodeURIComponent(title)}&price=${encodeURIComponent(price)}`,
    };
  });
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f7f4ef", color: "#1c1917" }}>
      <div style={{ width: 1240, margin: "0 auto", padding: "34px 0 80px" }}>
        ...
        >
          <ArrowLeft size={18} /> Tillbaka till sortiment
        </Link>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", gap: 24 }}>
            <div>
              ...
              <p style={{ margin: "14px 0 0", maxWidth: 760, fontSize: 16, lineHeight: 1.7, color: "#57534e" }}>
                Här visas fler produkter från denna avdelning. Klicka på en produkt för att beställa just den direkt.
              </p>
            </div>

            <Link
              href={`/order/private/guest-v4?floristId=${id}&category=${encodeURIComponent(decodedCategory)}`}
              style={{
                height: 52,
                display: "inline-flex",
                alignItems: "center",
            ...
            >
              <ShoppingBag size={18} /> Beställ från {decodedCategory}
            </Link>
          </div>
        </section>
        ...
        >
          {products.map((item) => (
            <Link key={item.id} href={item.href} style={{ color: "#1c1917", textDecoration: "none" }}>
              ...
              >
                <div style={{ height: 250, background: "#f5f5f4" }}>
                  <img
                    src={item.image}
                    alt={item.title}
