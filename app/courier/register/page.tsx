...
const vehicleTypes = ["Bil", "Elbil", "Skåpbil", "Lastbil", "Cykelbud", "Annat"];
const invoiceMethods = ["E-post", "E-faktura", "Brev"];

export default function CourierRegisterPage() {
  const [selectedVehicles, setSelectedVehicles] = useState<string[]>(["Bil"]);
  const [message, setMessage] = useState("");
  const [partnerNumber] = useState(() => generatePartnerNumber("FSBUD"));
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    window.localStorage.setItem("floristsocial_fsbud_profile", JSON.stringify({ partnerNumber, createdAt: new Date().toISOString(), profileRoute: "/courier/profile" }));
    setMessage("✅ Budfirma registrerad lokalt. Nästa steg är Supabase-koppling och profilgenerering.");
  }
        ...
        <section className="overflow-hidden rounded-[2rem] bg-white shadow-xl shadow-slate-200/80">
          <div className="bg-gradient-to-br from-pink-50 via-white to-emerald-50 px-6 py-8 sm:px-10">
            <p className="mb-3 inline-flex rounded-full bg-pink-100 px-3 py-1 text-xs font-black uppercase tracking-[0.18em] text-pink-700">
              FloristSocial partner
            </p>
            <h1 className="text-3xl font-black tracking-tight sm:text-5xl">
              Registrera budfirma
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
              Budfirmor kan beställa blommor åt sina kunder och få partnerersättning
              från FloristSocial.
            </p>
          </div>
          ...
          <form onSubmit={submit} className="grid gap-8 px-6 py-8 sm:px-10 lg:grid-cols-[1fr_340px]">
            <div className="space-y-6">
              ...
              <Card title="Företagsuppgifter" icon={<Building2 size={20} />}>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Organisationsnummer *" icon={<FileText size={18} />} />
                  <Field label="Företagsadress *" icon={<MapPin size={18} />} />
                  <Field label="Fakturaadress *" icon={<CreditCard size={18} />} />
                  <Field label="Kontaktperson *" icon={<User size={18} />} />
                  <Field label="Telefonnummer *" icon={<Phone size={18} />} />
                </div>
              </Card>

              <Card title="Leverans och fordon" icon={<Truck size={20} />}>
                  ...
                  {vehicleTypes.map((item) => (
                    <button
                      ...
                      className={`rounded-2xl border px-4 py-3 text-left text-sm font-black ${
                        selectedVehicles.includes(item)
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <Field label="Leveransområde *" placeholder="Stockholm, Solna, Nacka..." />
                  <Select label="Faktureringssätt" options={invoiceMethods} />
                </div>
              </Card>
            ...
            <aside className="rounded-[2rem] bg-slate-50 p-6">
              <h2 className="text-xl font-black">Vad händer sedan?</h2>
              <p className="mt-3 text-sm font-semibold leading-6 text-slate-600">
                FloristSocial granskar kontot. Efter godkännande får budfirman
                profil, fakturahantering och partnerersättning.
              </p>
            </aside>
          </form>
        </section>
...
function Field({ label, type = "text", placeholder = "", icon }: { label: string; type?: string; placeholder?: string; icon?: React.ReactNode }) {
  return <label className="block"><span className="mb-2 block text-sm font-black">{label}</span><div className="flex h-12 items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4">{icon}<input type={type} placeholder={placeholder} className="w-full bg-transparent text-sm font-bold outline-none" /></div></label>;
}

function Select({ label, options }: { label: string; options: string[] }) {
  return <label className="block"><span className="mb-2 block text-sm font-black">{label}</span><select className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-bold outline-none">{options.map((x) => <option key={x}>{x}</option>)}</select></label>;
}
