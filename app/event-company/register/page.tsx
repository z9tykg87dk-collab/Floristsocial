...
export default function EventCompanyRegisterPage() {
  const [selected, setSelected] = useState<string[]>(["Bröllop"]);
  const [message, setMessage] = useState("");
  const [partnerNumber] = useState(() => generatePartnerNumber("FSEVT"));
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    window.localStorage.setItem("floristsocial_fsevt_profile", JSON.stringify({ partnerNumber, createdAt: new Date().toISOString(), profileRoute: "/event-company/profile" }));
    setMessage("✅ Eventföretag registrerat lokalt. Nästa steg är Supabase-koppling och profilgenerering.");
  }
        ...
        <section className="overflow-hidden rounded-[2rem] bg-white shadow-xl shadow-slate-200/80">
          <div className="bg-gradient-to-br from-pink-50 via-white to-emerald-50 px-6 py-8 sm:px-10">
            ...
            <h1 className="text-3xl font-black tracking-tight sm:text-5xl">
              Registrera eventföretag
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
              Eventföretag kan beställa blommor till kunder och få partnerersättning
              via FloristSocial.
            </p>
          </div>
          ...
          <form onSubmit={submit} className="grid gap-8 px-6 py-8 sm:px-10 lg:grid-cols-[1fr_340px]">
            <div className="space-y-6">
              ...
              <Card title="Företagsuppgifter" icon={<Building2 size={20} />}>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Stad *" icon={<MapPin size={18} />} />
                </div>
              </Card>

              <Card title="Eventområden" icon={<CalendarDays size={20} />}>
            ...
            <aside className="rounded-[2rem] bg-slate-50 p-6">
              <PartyPopper className="text-pink-700" size={32} />
              <h2 className="mt-4 text-xl font-black">Partnerprofil</h2>
              <p className="mt-3 text-sm font-semibold leading-6 text-slate-600">
                Efter godkännande får eventföretaget en intern profil, ekonomi,
                fakturahantering och möjlighet att kommunicera med florister.
              </p>
            </aside>
          </form>
        </section>
