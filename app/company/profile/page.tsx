        ...
        <div className="flex flex-col gap-5 bg-gradient-to-r from-pink-100 via-rose-50 to-emerald-50 px-6 py-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-1 flex-col gap-3 lg:max-w-2xl">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <HeaderAction icon={<FileText size={17} />} label="Fakturor" count="2" />
      ...
      <div className="mx-auto max-w-7xl">
            ...
            <div className="-mt-16 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
                <div className="grid h-32 w-32 place-items-center rounded-[2rem] border-4 border-white bg-pink-100 text-pink-700 shadow-lg">
                  {company.logoUploaded ? <Building2 size={58} /> : <Upload size={50} />}
                </div>
        ...
        <section className="mt-8 rounded-[2rem] bg-white p-5 shadow-xl shadow-slate-200/70 sm:p-6">
          ...
          <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <CalendarShortcut href="#orderhistorik" label="Order" icon={<ShoppingBag size={17} />} />
            <CalendarShortcut href="#leveransadresser" label="Leverans" icon={<Truck size={17} />} />
            <CalendarShortcut href="#presentkort" label="Presentkort" icon={<Gift size={17} />} />
            <CalendarShortcut href="#bekraftelse" label="Bekräftelse" icon={<CheckCircle2 size={17} />} />
            <CalendarShortcut href="#hogtid" label="Högtid" icon={<Star size={17} />} />
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="space-y-6">
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <QuickCard icon={<ShoppingBag />} title="Företagsordrar" value="3 ordrar" />
              <QuickCard icon={<FileText />} title="Fakturor" value="2 aktiva" />
              <QuickCard icon={<Truck />} title="Leveranser" value="3 adresser" />
              <QuickCard icon={<CalendarDays />} title="Kalender" value="57 dagar" />
            </section>

            <ProfileCard id="foretagsuppgifter" title="Företagsuppgifter" icon={<Building2 size={22} />}>
              <div className="grid gap-4 sm:grid-cols-3">
                <InfoTile label="Företagsnamn" value={company.name} editable={isEditing} />
                <InfoTile label="Organisationsnummer" value={company.orgNumber} editable={isEditing} />
                <InfoTile label="VAT-nummer" value={company.vatNumber} editable={isEditing} />
                <InfoTile label="Bransch" value={company.industry} editable={isEditing} />
                <InfoTile label="Företagsrabatt" value={company.discount} editable={isEditing} />
                <InfoTile label="Prenumeration" value={company.activeSubscription ? "Aktiv" : "Ej aktiv"} editable={isEditing} />
              </div>
            </ProfileCard>

            <ProfileCard id="bekraftelse" title="Faktura & bokföring" icon={<FileText size={22} />}>
              <div className="grid gap-4 sm:grid-cols-2">
                <InfoTile label="Faktura-e-post" value={company.invoiceEmail} editable={isEditing} />
                <InfoTile label="Betalningsvillkor" value={company.paymentTerms} editable={isEditing} />
                <InfoTile label="Kreditgräns" value={company.creditLimit} editable={isEditing} />
                <InfoTile label="Kostnadsställe" value={company.costCenter} editable={isEditing} />
                <InfoTile label="Fakturareferens" value={company.accountingReference} editable={isEditing} />
                <InfoTile label="Admin-status" value={company.adminReview} editable={isEditing} />
              </div>
            </ProfileCard>

            <ProfileCard id="hogtid" title="Jag önskar att beställa" icon={<Sparkles size={22} />}>
              <div className="flex flex-wrap gap-2">
                {wishes.map((wish) => (
                  <span key={wish} className="rounded-full bg-pink-50 px-4 py-2 text-sm font-black text-pink-700">
                    {wish}
                  </span>
                ))}
              </div>
            ...
            <ProfileCard id="orderhistorik" title="Orderhistorik" icon={<PackageCheck size={22} />}>
          ...
          <aside className="space-y-6 lg:sticky lg:top-6 lg:self-start">
            <ProfileCard title="Kontaktperson" icon={<Users size={22} />}>
              <div className="space-y-3 text-sm">
                <ContactRow icon={<Users size={17} />} value={`${company.contactName}, ${company.contactRole}`} />
                <ContactRow icon={<Mail size={17} />} value={company.contactEmail} />
                <ContactRow icon={<Phone size={17} />} value={company.contactPhone} />
              </div>
            </ProfileCard>

            <ProfileCard title="Företagets kontakt" icon={<Home size={22} />}>
              <div className="space-y-3 text-sm">
                <ContactRow icon={<Mail size={17} />} value={company.companyEmail} />
                <ContactRow icon={<Phone size={17} />} value={company.phone} />
                <ContactRow icon={<MapPin size={17} />} value={`${company.address}, ${company.postalCode} ${company.city}`} />
              </div>
            </ProfileCard>

            <ProfileCard id="presentkort" title="Fakturakund" icon={<ShieldCheck size={22} />}>
              <div className="rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                ...
                <p className="mt-1">
                  AI-förkontroll är markerad som: {company.aiCreditCheck}. Fördjupad kontroll sker via Admin/Super Admin.
                </p>
              </div>
              <Link href="/company/interest" className={`mt-4 w-full ${primaryButtonClass}`}>
                Skicka intresseanmälan
              </Link>
            </ProfileCard>

            <ProfileCard id="leveransadresser" title="Leveransadresser" icon={<Truck size={22} />}>
              <div className="space-y-2">
                {deliveryAddresses.map((address) => (
                  <div key={address} className="rounded-2xl bg-slate-50 px-4 py-3 text-sm font-black text-slate-700">
                    {address}
                  </div>
                ))}
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4 py-6 backdrop-blur-sm">
            <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] border border-pink-100 bg-[#fffaf7] p-5 shadow-2xl sm:p-6">
                ...
                <div className="mb-5 rounded-3xl bg-white p-4 shadow-sm ring-1 ring-pink-100">
                  ...
                  <div className="space-y-2">
                    {selectedDayEvents.map((event) => (
                      <div key={`${event.date}-${event.title}`} className={`rounded-2xl border px-4 py-3 text-sm font-bold ${getEventStyle(event.type)}`}>
                        <p className="font-black">{event.title}</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {(["Beställa", "Prenumerera", "Event", "Sorgbukett", "Presentkort", "Faktura", "Påminna", "Jubileum"] as CalendarActionType[]).map((action) => (
                  <button
                    key={action}
                    type="button"
                  ...
                  >
                    {action}
                    <span className={`mt-1 block text-xs font-semibold ${activeAction === action ? "text-pink-100" : "text-slate-500"}`}>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                <Link href="/company/register" className="rounded-full bg-pink-600 p-4 text-center text-sm font-black text-white shadow-lg shadow-pink-200 hover:bg-pink-700">
                  Gå till beställning
                </Link>
                <Link href="/company/interest" className="rounded-full bg-pink-600 p-4 text-center text-sm font-black text-white shadow-lg shadow-pink-200 hover:bg-pink-700">
                  Intresseanmälan
                </Link>
                <Link href="/company/register" className="rounded-full bg-emerald-600 p-4 text-center text-sm font-black text-white shadow-lg shadow-emerald-100 hover:bg-emerald-700">
          ...
          <div className="grid gap-8 px-6 py-8 md:grid-cols-4">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Företagsprofil för beställningar, fakturor, kalender, event och återkommande blomsterleveranser.
              </p>
            </div>
            <FooterCol title="Företag" items={["Profil", "Orderhistorik", "Fakturor", "Bokföring"]} />
            <FooterCol title="Beställningar" items={["Kontorsblommor", "Event", "Sorgbukett", "Presentkort"]} />
            <FooterCol title="Support" items={["Admin kontakt", "Fakturakund", "Villkor", "Integritet"]} />
          </div>
        </footer>
      </div>
    </main>
...
function ProfileCard({ id, title, icon, children }: { id?: string; title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <section id={id} className="scroll-mt-24 rounded-[1.75rem] bg-white p-5 shadow-sm sm:p-6">
...
function InfoTile({ label, value, editable = false }: { label: string; value: string; editable?: boolean }) {
  return (
...
function CalendarShortcut({ href, label, icon }: { href: string; label: string; icon: React.ReactNode }) {
  return (
    <Link
      href={href}
...
function FooterCol({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
      <p className="text-sm font-black uppercase tracking-[0.14em] text-slate-400">{title}</p>
