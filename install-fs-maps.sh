...
cp components/FloristsMapView.tsx components/FloristsMapView.backup-before-fs-map.tsx

cat > components/FSMap.tsx <<'MAPEOF'
"use client";
          ...
          return (
            ...
            >
              {detailed ? (
                ...
                <div
                  className={`h-3 w-3 rounded-full border border-white bg-stone-400 shadow ${
                    isHovered ? "scale-125" : ""
                  }`}
                />
              )}
...
path = Path("components/FloristsMapView.tsx")
text = path.read_text()
...
text = text.replace("<FloristSearchMap", "<FSMap")
text = text.replace("</FloristSearchMap>", "</FSMap>")
